...
TMB_model <- function(cases, ndvi_and_cov, weights, startindex, n_pixels_vec) {
  ...
  parameters <- list(intercept = 0, slope = 0)
  ...
  # These extract all the "REPORT()s" from the model 
  ...
  results_list <- list(
    obj = obj,                                        # these increase memory output so comment out as needed
    opt = opt,                                        #
...
ci_lower <- function(b, se.b, alpha){
  z_quantile <- qnorm(1 - alpha) # qnorm(1 - alpha) = 1.96
...
zipper_plot <- function(parameter_name, true_value, data, n, index) {
  ...
  # Using the estimates data, we are simply selecting the columns for the parameter
  est <- data[, c(parameter_name, paste0("ci_lower_", parameter_name), 
                  paste0("ci_upper_", parameter_name), paste0("se_", parameter_name))]
  est$Simulation <- 1:n
  
  # Checking if the true value of parameter is within the CI
  # IF CI_lower <= true value AND CI_upper >= true value, THEN label TRUE. Otherwise, False.
  ...
  # Compute zi for parameter: z_i = (b_i - b.true) / se.b
  # z-score 
  ...
  # Rank the CIs based on the absolute value of zi
